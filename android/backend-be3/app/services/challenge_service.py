...
- `as_of` is an inclusive cutoff on each participant's local date (default:
  server UTC date today). Daily series are zero-filled for the window so the
  client can render charts deterministically.
...
def _compute_participant_scores(
    ...
    series = _series(window_start, window_end, as_of, scores_by_user.get(participant.user_id, {}), zero_fill)
...
def participant_totals(
    ...
    """Per-participant progress totals (as of server UTC today), sorted desc."""
    as_of = _utc(now).date()
    scores_by_user = _score_rows_by_user(db, challenge, [p.user_id for p in participants])
    rows = []
    for p in participants:
        series, total = _compute_participant_scores(db, challenge, p, scores_by_user, as_of, zero_fill=False)
        rows.append(
            {
                "user_id": p.user_id,
...
def compute_leaderboard(
    ...
    for p in participants:
        series, total = _compute_participant_scores(
            db, challenge, p, scores_by_user, cutoff, zero_fill=True
        )
        entries.append(
            {
                "user_id": p.user_id,
